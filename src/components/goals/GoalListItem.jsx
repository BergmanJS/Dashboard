...
const GoalTools = styled.div`
    ...
    transform: ${props => props.showTools ? 'translateY(0rem)' : 'translateY(2rem)' };
...
const deleteGoal = (goalId) => {
      firebase.database().ref('goals/' + goalId).remove();
}
  ...
  return (
    <GoalListItem onClick={() => handleShowTools()} >
        ...
        <GoalTools showTools={showTools}>
            <ToolBtn onClick={() => handleDeleteGoal(goalId)} className='mdi mdi-delete-outline'></ToolBtn>
            <ToolBtn onClick={() => handleCloseTools()} className='mdi mdi-close'></ToolBtn>
        </GoalTools>
    </GoalListItem> 
  );
};
